"""Fetch target journal metadata from OpenAlex and save normalized results."""
...
def fetch_journal(source_id: str, session: requests.Session) -> dict[str, Any]:
    """Query OpenAlex for a single journal by source identifier with retries."""
    ...
    while True:
        ...
        try:
            ...
            if attempt > MAX_RETRIES:
                raise JournalFetchError(f"request failed for source {source_id}: {exc}") from exc
            delay = BASE_BACKOFF_SECONDS * (2 ** (attempt - 1))
            LOGGER.warning(
                "Request error for source %s on attempt %s/%s: %s; retrying in %.1fs",
                source_id,
                attempt,
                MAX_RETRIES + 1,
                exc,
        ...
        if response.status_code == 429 or 500 <= response.status_code < 600:
            ...
            LOGGER.warning(
                "OpenAlex returned HTTP %s for source %s on attempt %s/%s; retrying in %.1fs",
...
def normalize_journal(result: dict[str, Any], fallback_issn: str, policy_start_year: int | None) -> dict[str, Any]:
...
def fetch_all_journals() -> tuple[list[dict[str, Any]], list[str]]:
    """Fetch metadata for all configured target journals."""
    ...
    with build_session() as session:
        for journal in TARGET_JOURNALS:
            ...
            policy_start_year = journal.get("policy_start_year")

            LOGGER.info("Fetching %s (%s, %s)", journal_name, issn, source_id)
            try:
                payload = fetch_journal(source_id=source_id, session=session)
                write_json(RAW_JOURNALS_DIR / f"{source_id}.json", payload)

                normalized = normalize_journal(
                    payload,
                    fallback_issn=issn,
                    policy_start_year=policy_start_year,
                )
                ...
                LOGGER.info(
                    ...
                    normalized.get("works_count"),
                )
            except JournalFetchError as exc:
                failures.append(source_id)
                LOGGER.error("Failed to fetch %s (%s): %s", journal_name, source_id, exc)
            except (OSError, ValueError, TypeError) as exc:
                failures.append(source_id)
                LOGGER.error("Failed to process %s (%s): %s", journal_name, source_id, exc)
            finally:
